var LichessDemo = (function () {
    ...
    const scopes = ['board:play', 'study:read', 'study:write'];
    ...
    class Auth {
        constructor() {
            ...
            this.fetchBodyParse = async (path, config = {}) => {
                const res = await this.fetchResponse(path, config);
...
[Date "2013.10.14"]
[Round "?"]
[White "12. Finales dos peones"]
[Black "?"]
[Result "*"]
[PlyCount "0"]

{Después de muchos capítulos, volvemos a los finales de peones, ahora para
tratar las posiciones con más de un peón en el tablero.  A pesar de lo
limitado del material, los finales de peones suelen ser muy complicados y
exigen una gran cantidad de cálculo. Tanto es así, que algunos jugadores
recomiendan realizar la ventaja, si es posible, sin trasponer a finales de
peones, en los cuales es fácil pasar por alto una sutileza táctica. Este
consejo de viejo zorro, se justifica si se miran las estadísticas, y se
comprueba que los finales de peones, son los que terminan menos veces en
tablas.  También es curioso que no haya demasiadas posiciones teóricas
importantes. Pero por el contrario sí existen muchos temas que es necesario
conocer. Por ese motivo este será el único capítulo en que analizaremos alguna
posición con muchos peones, porque es la única forma de ilustrar bien algún
tema que considero básico.}  *
...
`;
    ...
    const li = (titulo, enlace) => h('li', h('a', {
        attrs: { href: enlace },
    }, titulo));
    const li2 = (follow) => h('li', follow);
    const estudioButtons = (ctrl, s) => {
    ...
    function confirmarSubida(s, pngs) {
    ...
    class Studies {
        ...
        async getStudies(root) {
            const url = `/api/study/by/${root.auth.me.id}`;
            let response;
            try {
                this.db = (await root.auth.fetchBodyParse(url, { method: 'get' }))
        ...
        async uploadStudies(root, s, pngs) {
        ...
        async subirStudio(root, s, partida) {
            const url = `/api/study/${s.id}/import-pgn`;
            console.log("Enviando a", url, "esto", partida);
            let response = { chapters: [] };
            try {
                response = (await root.auth.fetchBodyParse(url, {
                    method: 'post',
                    body: formData(Object.assign(Object.assign({}, partida), { keepAliveStream: false }))
                }));
        ...
        constructor(redraw) {
            ...
            this.openHome = async () => {
                ...
                if (this.auth.me) {
                    ...
                    this.games.empty();
                    this.studies.empty();
                    await this.studies.getStudies(this);
                    /*DEV TEMPORAL  this.stream = await this.auth.openStream('/api/stream/event', {}, msg => {
                      switch (msg.type) {
                        case 'gameStart':
    ...
    const renderNavBar = (ctrl) => h('header.navbar.navbar-expand-md.navbar-dark.bg-dark', [
        h('div.container', [
            h('a.navbar-brand', {
                attrs: href('/'),
            }, 'AlephScript PGN Lichess Utilidad'),
    ...
    const userHome = (ctrl) => [
        h('div', [
            ...
            h('h2.mt-5.mb-3', 'Cortadora/Exportadora de PNGs'),
            h('div.games', renderCortadora(ctrl)),
    ...
    const anonHome = () => [
        h('div.login.text-left', [
            renderAbout(),
            h('div.big', [h('p', '¿Te autorizas  en lichess?')]),
