var LichessDemo = (function () {
    ...
    const finalitos = [
        'finalitos/F13-Torre-contra-Alfil.pgn',
        'finalitos/Final/12-Finales-dos-peones-.pgn'
    ];
    const li = (titulo, enlace) => h('li', h('a', {
        attrs: { href: enlace },
    }, titulo));
    const renderCortadora = () => h('div.cortadora', [
        h('p', 'Este es la utilidad La Cortadora de PNGs.'),
        h('ul', finalitos.map(f => li(f, f))),
        h('p', [
            'Esto que tienes arriba está listo para ser exportado a Estudio',
            h('code', '<Ctrl+Shift+j+palabra_mágica>'),
            ' para subir un estudio.',
            h('br'),
            'Si algo se rompe es culpa tuya por no revisar el código.',
        ])
    ]);
    ...
    const anonHome = () => [
        h('div.login.text-center', [
            h('div.big', [renderCortadora()]),
            h('div.big', [renderAbout()]),
